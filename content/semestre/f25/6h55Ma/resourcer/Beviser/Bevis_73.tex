...
\subsection*{Logaritmeregnereglen $\log(ab)=\log(a)+\log(b)$}
\hspace{1em} \circnum{1} Opskriv definitionen af logaritmen for $a$ og $b$:
$$a = 10^{\log(a)}\quad\text{og}\quad b = 10^{\log(b)}$$


\hspace{1em} \circnum{2} Opskriv udtryk for $a\cdot b$: 
\begin{align*}
    a\cdot b& = 10^{\log(a)} \cdot 10^{\log(b)}\\ 
    a\cdot b& = 10^{\log(a)+\log(b)}\\ 
\intertext{\hspace{1em} \circnum{3} Tag logaritmen på begge sider af lighedstegnet:}
    \log(a\cdot b)& = \log\left(10^{\log(a)+\log(b)}\right)\\ 
