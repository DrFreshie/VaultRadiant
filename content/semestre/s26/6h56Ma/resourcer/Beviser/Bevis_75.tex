...
\subsection*{Logaritmeregnereglen $\log(a^r)=r\cdot \log(a)$}
\hspace{1em} \circnum{1} Opskriv definitionen af logaritmen for $a$:
\begin{align*}
a &= 10^{\log(a)}\\
\intertext{\hspace{1em} \circnum{2} Opløft begge sider af lighedstegnet med $r$:}
a^r &= \left(10^{\log(a)}\right)^r\\
\intertext{\hspace{1em} \circnum{3} Tag logaritmen på begge sider af lighedstegnet:}
\log\left(a^r\right) &= \log\left(\left(10^{\log(a)}\right)^r\right)\\
\log\left(a^r\right) &= \log\left(10^{\log(a)\cdot r}\right)\\
\log\left(a^r\right) &= \log(a)\cdot r
\end{align*}
