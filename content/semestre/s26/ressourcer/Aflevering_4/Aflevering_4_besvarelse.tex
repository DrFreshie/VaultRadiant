...
\svar{
    ...
    Formlen for afstanden mellem punkt og linje er:
    $$dist(P,l)=\frac{|a x_1+b-y_1|}{\sqrt{a^2+1}}.$$
    Indsættes de kendte værdier i denne får vi:
...
\end{enumerate}
\svar{
    Bruger vi formlen for afstanden mellem punkt og linje får vi:
    $$dist(P,l)=\frac{|a x_1+b-y_1|}{\sqrt{a^2+1}},\quad \text{med } a=2,\;b=3, \text{og }\;P(x_1,y_1)=(4,1).$$
    $$dist(P,l)=\frac{|2\cdot 4+3-1|}{\sqrt{2^2+1}}
        =\frac{10}{\sqrt{5}}
        =2\sqrt{5}\approx 4{,}47.$$
        Altså er afstanden $$dist(P,l)=4{,}47$$
}
...
\svar{
    ...
    Vi indsætter $y$ i cirklen og får:
    \begin{align*}
      (x-4)^2+(2x+3-1)^2&=25\\
      (x-4)^2+(2x+2)^2&=25\\
    ...
    Altså er $x=\pm 1$. $y$-værdien kan så findes ved at indsætte disse værdier i $y=2x+3$:
    \begin{align*}
      y_1&=2\cdot 1+3=5 \text{, og }\\
      y_2&=2\cdot (-1)+3=1
    \end{align*}

      og koordinatsættene til skæringspunktene bliver således:
