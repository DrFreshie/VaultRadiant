...
\subsection{}
\begin{enumerate}
\item Find hældningen mellem punkterne $A(1,3)$ og $B(5,11)$.
\item Find hældningen mellem $A(-2,4)$ og $B(3,-6)$.
\item Find hældningen mellem $A(0,-1)$ og $B(4,-1)$.
\item Find hældningen mellem $A(2,5)$ og $B(2,-3)$. Hvad kan man sige her?
\item En linje går gennem $A(1,2)$ og $B(4,8)$. Bestem linjens ligning $y=ax+b$.
\item En linje går gennem $A(-3,7)$ og $B(1,-1)$. Bestem $y=ax+b$.
\item Givet $f(x)=2x-5$: beregn $f(-2)$, $f(0)$, $f(3)$.
\item Givet $g(x)=x^2-4x+1$: beregn $g(0)$, $g(2)$, $g(5)$.
\item Givet $h(x)=\frac{1}{2}x+3$: find $h(4)$ og løs $h(x)=8$.
\item En funktion har $f(1)=4$ og $f(5)=12$. Find den gennemsnitlige ændringshastighed på intervallet $[1,5]$.
\item For $f(x)=x^2$: find gennemsnitlig ændringshastighed på $[1,3]$.
\item For $f(x)=x^2$: find gennemsnitlig ændringshastighed på $[2,2{,}5]$.
\item Sammenlign svarene i 11 og 12. Hvad fortæller det om hældning for en krum graf?
\item Givet $f(x)=3x-2$: beregn $\frac{f(x+h)-f(x)}{h}$ for $h\neq 0$.
\item Givet $f(x)=x^2$: beregn $\frac{f(x+h)-f(x)}{h}$ og forenkl.
\end{enumerate}
