...
\documentclass[10pt,a4paper,table]{article}
  ...
  \usepackage{transparent}
	...
	\fancypagestyle{plain}{%
		...
		\fancyhead[L]{}
		\fancyhead[C]{}
		\fancyhead[R]{\overp\color{frbgreenl}\normalsize \overp Mat C-B\\\huge\color{frbgreen}\vestb Opvarmning}
		\cfoot{\thepage}
	}
	\pagestyle{fancy}
	\fancyhf{}
...
\item $\frac{-24}{-3}-5$
\item $-2(3-5)+4$
\item $\left(-\frac{1}{2}\right)(12)+(-3)^2$
\item $(-3)^3-2(-3)^2+5$
\end{enumerate}
\item Hvis $c=-4$, hvad er $-c$? Hvis $c=-0,3$, hvad er $-c$?
\item Linjen $y=2x+1$: hvad er ændringen i $y$, når $x$ stiger med $1$?
\item Linjen $y=-3x+5$: hvad er ændringen i $y$, når $x$ stiger med $1$?
\item Givet punkterne $(1,3)$ og $(4,9)$: find hældningen.
\item Givet punkterne $(-2,5)$ og $(2,-3)$: find hældningen.
\item En retvinklet trekant har kateter $a=1$ og $b=4$. Find hypotenusen.
\item En retvinklet trekant har kateter $a=1$ og $b=3$. Find hypotenusen.
\item Reducér udtrykket: $(a-c)^2+1$.
\item Reducér: $(a-c)^2+1-(a^2+1)-(c^2+1)$.
\item Løs ligningen $-2ac=2$ for produktet $ac$.
\end{enumerate}
