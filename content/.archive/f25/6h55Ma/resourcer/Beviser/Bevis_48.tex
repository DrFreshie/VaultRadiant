\documentclass[10pt,a4paper]{article}
	...
	\fancypagestyle{plain}{%
		...
		\fancyhead[C]{\overp\color{frbblue}\normalsize 6H56 Ma: mundtlig eksamen\\\Large\vestb\color{frbgreen} Beviser}
		\cfoot{\thepage}
	}
	\pagestyle{fancy}
	\fancyhf{}
	\renewcommand\headrule{
			...
			\end{minipage}}}
	\lhead{\klavl\color{frbgreen}\footnotesize 6H56 Ma}
	\rhead{\klavl\color{frbgreen}\footnotesize Forår 2026}
	\cfoot{\thepage}
...
\subsection*{Formlen for hældninger mellem ortogonale linjer $a\cdot c=-1$}
\hspace{1em} \circnum{1} Tegn figur:
\begin{figure}[h!]
\centering
\includegraphics[width=0.4\textwidth]{ortogonal_1}
\end{figure}

\hspace{1em} \circnum{2} Beskriv de to retvinklede trekanter og opskriv udtryk for deres hypotenuser:
$$h_1=\sqrt{1^2+(-c)^2}\quad\text{og}\quad h_2=\sqrt{1^2+a^2}$$
\hspace{1em} \circnum{3} Tilføj den store trekant på figuren:
\begin{figure}[h!]
\centering
\includegraphics[width=0.4\textwidth]{ortogonal_2}
\end{figure}

\hspace{1em} \circnum{3} Opskriv udtryk for $(a-c)^2$ og "isolér"\ $ac$:
\begin{align*}
(a-c)^2&=h_1^2+h_2^2\\
(a-c)^2&=\left(\sqrt{1^2+(-c)^2}\right)^2+\left(\sqrt{1^2+a^2}\right)^2\\
a^2+c^2-2ac&=1^2+(-c)^2+1^2+a^2\\
a^2+c^2-2ac&=a^2+c^2+2\\
-2ac&=a^2-a^2+c^2-c^2+2\\
ac&=\frac{2}{-2}\\
ac&=-1
\end{align*}
