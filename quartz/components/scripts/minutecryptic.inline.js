...
function parseMinuteCrypticKeyValueBlock(input) {
  const params = {}
...
function escapeMinuteCrypticHtml(value) {
  return value
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function splitHighlightParts(value) {
  return value
    .split("|")
    .map((part) => part.trim())
    .filter(Boolean)
}
function findHighlightRanges(clue, snippet) {
  const ranges = []
...
function renderHighlightedClue(state) {
  const highlights = [
...
function getMinuteCrypticState(container) {
  return container._minuteCrypticState
}
function getFilledCount(state) {
  return state.guess.filter(Boolean).length
}
function getCurrentGuess(state) {
  return state.guess.join("")
}
function getNextEmptyIndex(state) {
  return state.guess.findIndex((char) => !char)
}
function getLastEditableFilledIndex(state) {
...
function renderGuessSlots(state) {
...
function renderMinuteCryptic(container, state) {
  const clue = container.querySelector(".minutecryptic-clue")
  const enumeration = container.querySelector(".minutecryptic-enumeration")
  const slots = container.querySelector(".minutecryptic-slots")
  const status = container.querySelector(".minutecryptic-status")
  const answer = container.querySelector(".minutecryptic-answer")
  const explanation = container.querySelector(".minutecryptic-explanation")
  const keyboardButtons = Array.from(container.querySelectorAll(".minutecryptic-key"))
  ...
  const backspaceButton = container.querySelector(".minutecryptic-backspace")
  if (backspaceButton) {
    backspaceButton.disabled =
      state.solved || state.revealed || getLastEditableFilledIndex(state) === -1
  }
  const clearButton = container.querySelector(".minutecryptic-clear")
  if (clearButton) {
    clearButton.disabled = state.solved || state.revealed || getFilledCount(state) === 0
  }
  const hintButton = container.querySelector(".minutecryptic-hint-letter")
  if (hintButton) {
    hintButton.disabled = state.solved || state.revealed || getNextHintIndex(state) === -1
  }
  const checkButton = container.querySelector(".minutecryptic-check")
  if (checkButton) {
...
function appendGuess(container, char) {
...
function backspaceGuess(container) {
  const state = getMinuteCrypticState(container)
  if (!state || state.solved || state.revealed) return
...
function initMinuteCryptic(container) {
  ...
  container.querySelectorAll(".minutecryptic-key").forEach((button) => {
    button.addEventListener("click", () => {
      appendGuess(container, button.dataset.letter ?? "")
    })
  })
...
function hydrateMinuteCrypticWidgets(root = document) {
  const widgets = Array.from(root.querySelectorAll(".minutecryptic-widget"))
  for (const widget of widgets) {
    if (widget._minuteCrypticState) continue
    initMinuteCryptic(widget)
  }
}
function upgradeMinuteCrypticPlaceholders(root = document) {
  ...
  for (const code of candidates) {
    ...
    const nearestFigure = pre.closest("figure[data-rehype-pretty-code-figure]")
...
window.upgradeMinuteCrypticPlaceholders = upgradeMinuteCrypticPlaceholders
window.hydrateMinuteCrypticWidgets = hydrateMinuteCrypticWidgets
function bootMinuteCryptic(root = document) {
  upgradeMinuteCrypticPlaceholders(root)
  hydrateMinuteCrypticWidgets(root)
}
window.bootMinuteCryptic = bootMinuteCryptic
