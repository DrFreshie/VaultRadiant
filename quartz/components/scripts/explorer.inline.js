...
function toggleExplorer() {
    ...
    nearestExplorer.setAttribute("aria-expanded", nearestExplorer.getAttribute("aria-expanded") === "true" ? "false" : "true");
...
function findFolderNode(root, folderSlug) {
    const segments = simplifySlug(folderSlug).split("/");
    let node = root;
    for (const seg of segments) {
        if (!seg || !node)
            continue;
        const next = node.children.find((child) => child.isFolder && child.slugSegment === seg);
        if (!next) {
            return null;
        }
        node = next;
...
async function setupExplorer(currentSlug) {
    ...
    for (const explorer of allExplorers) {
        ...
        const oldIndex = new Map(serializedExplorerState.map((entry) => [entry.path, entry.collapsed]));
        ...
        if (simpleCurrent === "/" || simpleCurrent === "index" || simpleCurrent === "") {
            ...
            const semestreNode = findFolderNode(trie, "semestre");
            if (semestreNode) {
            ...
            const classSlug = parts.slice(0, semestreIdx + 3).join("/");
            ...
            const classSlug = parts.slice(0, classnotesIdx + 2).join("/");
            const maybeClass = findFolderNode(trie, classSlug);
            if (maybeClass)
                rootNode = maybeClass;
        }
