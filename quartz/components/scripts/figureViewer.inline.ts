function setupFigureViewer() {
  const panel = document.querySelector<HTMLDivElement>(".figure-viewer-panel")
  if (!panel) return

  const triggers = Array.from(document.querySelectorAll<HTMLElement>("article .fig-trigger"))
  ...
  let showTriggerEl: HTMLElement | null = null
  ...
  function setContent(figSrc: string | null, iframeSrc: string | null) {
    panel.innerHTML = ""
    if (!figSrc && !iframeSrc) return
  ...
  function applyPosition() {
    const wrapper = panel.querySelector<HTMLElement>(".figure-viewer__content")
    ...
    if (exitTriggerEl) {
      ...
      if (wrapper) wrapper.style.transform = `translateY(${-progress * 100}vh)`
      if (progress >= 1) {
        panel.innerHTML = ""
        exitTriggerEl = null
      }
    ...
    if (!wrapper || !showTriggerEl) return
  ...
  function getActive(): { key: string | null; figSrc: string | null; iframeSrc: string | null; el: HTMLElement | null } {
    ...
    for (const trigger of triggers) {
      ...
      if (trigger.getBoundingClientRect().top <= threshold) active = trigger
    }
