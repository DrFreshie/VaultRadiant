...
function addClipboard() {
    ...
    for (let i = 0; i < els.length; i++) {
        const button = els[i].querySelector(".clipboard-button");
...
function renderPopoverInSlide() {
    const mindmapLinks = document.querySelectorAll(".remark-slide-content a.internal");
...
function renderMermaidInSlide() {
    requestAnimationFrame(async () => {
        ...
        if (codeBlocks.length === 0)
            return;
        const { default: mermaid } = await import("https://cdnjs.cloudflare.com/ajax/libs/mermaid/11.4.0/mermaid.esm.min.mjs");
        mermaid.initialize({
            startOnLoad: false,
...
function enableYouTubeJSAPI(html) {
...
function anchorBlank(html) {
    // return html.replace(/<a\b([^>]*?)>/g, '<a $1 target="_blank">')
    // return html.replace(/(?<!<sup>)<a\b([^>]*?)>/g, '<a $1 target="_blank">')
    return html;
}
function unwrapSlideNote(html) {
    return html.replace(/<p>(\?{3}[\s\S]*?)<\/p>/g, (_, content) => content);
}
function unwrapFootnotesSection(html) {
    return html.replace(/<section[^>]*data-footnotes[^>]*class="footnotes"[^>]*>([\s\S]*?)<\/section>/gi, (_, inner) => inner);
}
function injectSeparators(html, separator) {
...
function handleFootnote(data, separator) {
    ...
    slides[slides.length - 1] = slides[slides.length - 1].replace(/<a\s+([^>]*?)href="(#user-content-fn[^"]+)"([^>]*)>/g, (fullMatch, beforeHref, href, afterHref) => {
...
function handleIndex(data, separator, option, index) {
    const slides = data.split(separator);
    ...
    if (slides.length > 2) {
        slides[1] = slides[1].replace(/<a\s+([^>]*?)data-href="([^"]*)" ([^>]*)>/g, (fullMatch, beforeHref, dataHref, afterHref) => {
...
function appendCSS(option) {
    if (document.getElementById("slide-align-style"))
        return;
...
function appendRemark(option) {
    ...
    const data = pipe(injectSeparators(header + (option.tags ? tags : "") + body, separator), (d) => handleIndex(d, separator, option, makeIndex()), (d) => handleFootnote(d, separator), unwrapFootnotesSection, unwrapSlideNote, enableYouTubeJSAPI, anchorBlank);
...
function paramOption(defaultOption) {
    const params = new URLSearchParams(window.location.search);
    const getBool = (key, fallback = false) => {
        const value = params.get(key)?.toLowerCase();
        return value === "true" ? true : value === "false" ? false : fallback;
    };
    const validRatios = ["4:3", "16:9"];
    const requestedRatio = params.get("ratio");
    const ratio = validRatios.includes(requestedRatio) ? requestedRatio : defaultOption.ratio;
    const validAlign = ["left", "center"];
    const requestedAlign = params.get("align");
    const align = validAlign.includes(requestedAlign) ? requestedAlign : defaultOption.align;
    return {
        ...defaultOption,
        ratio,
        navigation: {
...
document.addEventListener("nav", async () => {
    async function renderSlide() {
        ...
        const option = slideContainers[0].dataset["cfg"];
        if (!option) {
