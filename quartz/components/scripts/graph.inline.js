import { forceSimulation, forceManyBody, forceCenter, forceLink, forceCollide, forceRadial, zoomIdentity, select, drag, zoom, } from "d3";
...
async function renderGraph(graph, fullSlug) {
    ...
    let { drag: enableDrag, zoom: enableZoom, depth, scale, repelForce, centerForce, linkDistance, fontSize, opacityScale, removeTags, showTags, focusOnHover, enableRadial, } = JSON.parse(graph.dataset["cfg"]);
    ...
    function renderLabels() {
        ...
        for (const n of nodeRenderData) {
            ...
            if (hoveredNodeId === nodeId) {
                tweenGroup.add(new Tweened(n.label).to({
                    alpha: 1,
