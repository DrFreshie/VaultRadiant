...
function parseKeyValueBlock(input) {
    const params = {};
...
function renderGame(container, state) {
    ...
    const word = container.querySelector(".hangman-word");
    const status = container.querySelector(".hangman-status");
    const wrong = container.querySelector(".hangman-wrong");
    const figure = container.querySelector(".hangman-figure");
    const answer = container.querySelector(".hangman-answer");
    if (word) {
    ...
    const buttons = Array.from(container.querySelectorAll(".hangman-key"));
...
function guessLetter(container, letter) {
    const state = container._hangmanState;
    if (!state)
        return;
    const normalized = normalizeLetter(letter);
    if (state.guessed.has(normalized))
        return;
    if (state.wrong >= state.maxWrong)
        return;
    if (getUniqueLetters(state.answer).every((char) => state.guessed.has(char)))
        return;
...
function initGame(container) {
  ...
  `;
    ...
    container.querySelectorAll(".hangman-key").forEach((button) => {
        button.addEventListener("click", () => guessLetter(container, button.dataset.letter ?? ""));
    });
    ...
    renderGame(container, container._hangmanState);
}
function hydrateHangmanWidgets(root = document) {
    const widgets = Array.from(root.querySelectorAll(".hangman-widget"));
    for (const widget of widgets) {
        if (widget._hangmanState)
            continue;
        initGame(widget);
    }
}
function upgradeHangmanPlaceholders(root = document) {
    const candidates = Array.from(root.querySelectorAll("pre > code"));
    for (const code of candidates) {
        const pre = code.parentElement;
        if (!pre || pre.dataset.hangmanUpgraded === "true")
            continue;
        ...
        const declaredLanguage = code.getAttribute("data-language") ??
            pre.getAttribute("data-language") ??
            nearestFigure?.getAttribute("data-language") ??
            "";
        const looksLikeHangmanBlock = declaredLanguage.toLowerCase() === "hangman" ||
            /(\bword\s*:|\banswer\s*:)/i.test(code.textContent ?? "");
        if (!looksLikeHangmanBlock)
            continue;
...
;
window.upgradeHangmanPlaceholders = upgradeHangmanPlaceholders;
window.hydrateHangmanWidgets = hydrateHangmanWidgets;
function bootHangman(root = document) {
    upgradeHangmanPlaceholders(root);
    hydrateHangmanWidgets(root);
}
;
window.bootHangman = bootHangman;
