function normalizeSidebarHeading(text: string) {
  return text.replace(/^[^\p{L}\p{N}]+/gu, "").trim().toLowerCase()
}

function moveLessonSectionToSidebar(sectionName: string, className: string) {
  ...
  if (!rightSidebar || !lessonContent) return null
  if (rightSidebar.querySelector(`.${className}`)) {
    return rightSidebar.querySelector(`.${className} .materials-sidebar-content`) as HTMLElement | null
  }

  const headings = Array.from(lessonContent.querySelectorAll(":scope > h2")) as HTMLHeadingElement[]
  const heading = headings.find(
    (candidate) => normalizeSidebarHeading(candidate.textContent ?? "") === sectionName,
  )
...
type WiktionaryApiResponse = Record<string, WiktionaryEntry[]>
...
const wiktionaryCache = new Map<string, Promise<DictionaryLookupResult>>()
const dictionaryStorageKey = "quartz-wiktionary-last-term"

function fetchWiktionary(term: string, lang: string) {
  ...
  const request = fetch(
    `https://${lang}.wiktionary.org/api/rest_v1/page/definition/${encodeURIComponent(normalizedTerm)}`,
  ).then(async (response) => {
    ...
    if (!entry || !definition) throw new Error("No definition found")
...
function absolutizeWiktionaryLinks(container: HTMLElement, lang: string) {
...
async function getDictionaryEntry(term: string, lang: string) {
  return fetchWiktionary(term, lang)
}

function renderDictionaryResult(host: HTMLElement, term: string, lang: string, html: string) {
  host.innerHTML = html
  host.dataset.term = term
  absolutizeWiktionaryLinks(host, lang)
}

function createDictionaryWidget(existingContent: HTMLElement | null) {
  const rightSidebar = document.querySelector(".sidebar.right") as HTMLElement | null
  if (!rightSidebar) return

  let panel = rightSidebar.querySelector(".dictionary-sidebar-section") as HTMLElement | null
  let content: HTMLElement | null = panel?.querySelector(".materials-sidebar-content") as HTMLElement | null

  if (panel?.querySelector(".dictionary-search-form")) return
  ...
  const input = form.querySelector(".dictionary-search-input") as HTMLInputElement | null
  if (!input) return

  const lang = "en"
  const submitSearch = async (rawTerm: string) => {
    ...
    renderDictionaryResult(
      ...
      term,
      lang,
      `<p class="wiktionary-status">Looking up <strong>${escapeHtml(term)}</strong>…</p>`,
    )
  ...
  document.addEventListener("pointerdown", (event) => {
    if (event.target === floatingButton) return
    hideFloatingButton()
  })
