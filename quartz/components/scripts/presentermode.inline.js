...
function getPresenterContentRoot(article) {
    return (article.querySelector(":scope > .lesson-section > .lesson-content") ??
        article.querySelector(":scope > .popover-hint") ??
        article);
}
function buildPresenterModel(contentRoot) {
...
function highlightActiveProgramItem(container, activeTitle) {
    ...
    if (!normalizedActiveTitle)
        return;
    const match = items.find((item) => {
        const itemText = normalizeHeading(item.textContent ?? "");
        return (itemText === normalizedActiveTitle ||
...
function renderLeftSidebar(programHtml, total, current, activeTitle) {
    ...
    `;
        const program = leftSidebar.querySelector(".presenter-program");
        if (program) {
...
async function animateModeTransition(updateMode) {
...
function isProgramSlideTransition(fromIndex, toIndex) {
    return (fromIndex === 0 && toIndex === 1) || (fromIndex === 1 && toIndex === 0);
}
async function animateProgramSlideTransition(updateUI) {
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduceMotion) {
        updateUI();
        return;
    ...
    const fadeOuts = targets.map((el) => el.animate([{ opacity: 1 }, { opacity: 0 }], {
        duration: 400,
        easing: "ease-out",
        fill: "forwards",
    }));
...
async function showActiveSection(index) {
    const sections = Array.from(document.querySelectorAll(".presenter-section"));
    ...
    const updateUI = () => {
        ...
        renderLeftSidebar(model?.programSection?.html ?? null, sections.length, activeSectionIndex, activeTitle);
    };
...
function applyPresenterMode() {
    ...
    contentRoot.innerHTML = `<div class="presenter-sections">${model.presenterSections
        .map((section, index) => `<section class="presenter-section${index === activeSectionIndex ? " active" : ""}" data-index="${index}">${index === 0 && lessonTitle ? `<div class="presenter-program-slide"><div class="presenter-lesson-title">${escapeHtml(lessonTitle)}</div><div class="presenter-program-card">${section.html}</div></div>` : section.html}</section>`)
        .join("")}</div>`;
    window.bootHangman?.(contentRoot);
    renderLeftSidebar(model.programSection?.html ?? null, model.presenterSections.length, activeSectionIndex, model.presenterSections[activeSectionIndex]?.title ?? null);
...
function handleSidebarClick(e) {
    ...
    if (!button)
        return;
    const isPresenterControl = button.classList.contains("presenter-close") ||
        button.classList.contains("presenter-prev") ||
        button.classList.contains("presenter-next");
    if (!isPresenterControl)
        return;
